...
import { useAuth, useGoogleAuth, useLogin, useLogout, useRegister } from "@/features/auth/hooks";
    ...
    <div className="grid min-h-[70vh] gap-6 px-6 py-16 lg:grid-cols-2">
      <section className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3 pt-2">
          ...
          <Button type="button" variant="secondary" onClick={() => navigate("/")}>Back Home</Button>
        </div>
      </section>

      <section className="grid gap-6">
        <form onSubmit={handleLogin} className="space-y-4 rounded-xl border bg-background p-5 shadow-sm">
          <div>
        ...
        <form onSubmit={handleRegister} className="space-y-4 rounded-xl border bg-background p-5 shadow-sm">
          <div>
            <h2 className="text-lg font-semibold">Register</h2>
            <p className="text-sm text-muted-foreground">Create a new account and workspace.</p>
          </div>
          <input
            className="h-10 w-full rounded-md border bg-background px-3"
            type="email"
          ...
          {registerMutation.isError ? (
            <p className="text-sm text-destructive">Registration failed. Try a different email or username.</p>
          ) : null}
